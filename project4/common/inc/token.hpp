...
#define DEF_IS_CONVERTABLE_FROM(type)\
	...
	// ptr.get() converts shared_ptr(or smart ptr) to regular ptr
...
// todo:
//template <typename T, typename U> inline bool == (T const& lhs, T const& rhs) { return lhs == rhs; }
...
inline bool is(ORIGINAL_TYPE const& tkn) {
	return dynamic_cast<CAST_TYPE*>(tkn.get()) != nullptr;
}
...
template <typename CAST_TYPE, typename ORIGINAL_TYPE>
inline bool is(ORIGINAL_TYPE const* tkn) {
	return dynamic_cast<CAST_TYPE*>(tkn) != nullptr;
}
